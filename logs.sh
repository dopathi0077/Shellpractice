...
echo "Script execution is started at :- $(date)" &>>$LOG_FILE


if [ $USERID -ne 0 ]
then
   echo "ERROR: $R Please run the script with root access $N " &>>$LOG_FILE
...
if [ $? -ne 0 ]
then 
  echo "nginx is going to install now " &>>$LOG_FILE
  dnf install nginx -y &>>$LOG_FILE
  VALIDATE $? "nginx"
else
   echo -e "Nginx is already $Y installed no changes to do $N !" &>>$LOG_FILE
fi 

dnf list installed mysql
if [ $? -ne 0 ]
then
    echo "$Y MySQL is not installed... going to install it $N " &>>$LOG_FILE
...
if [ $? -ne 0 ]
then
    echo "$Y python3 is not installed... going to install it $N " &>>$LOG_FILE
